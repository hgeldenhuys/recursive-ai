...
Run the next-id command to get a unique story ID:
```bash
bun $RECURSIVE_AI_PATH/packages/cli/src/index.ts next-id .swarm/config.md
```

Parse the JSON output to get the `id` field for the new story.
...
Run the validation command to verify the story is well-formed:
```bash
