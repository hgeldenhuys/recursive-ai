...
// packages/core/src/frontmatter/parser.ts
...
// packages/core/src/frontmatter/validator.ts
...
var COMPLEXITIES = ["trivial", "simple", "moderate", "complex", "epic"];
var AC_STATUSES = ["pending", "passing", "failing"];
var TASK_STATUSES = ["pending", "in_progress", "done", "skipped"];
function validateStoryMeta(meta) {
  const errors2 = [];
  if (!isString(meta.id))
    errors2.push(error("id", "must be a string"));
  if (!isString(meta.title))
    errors2.push(error("title", "must be a string"));
  if (!isString(meta.status) || !STORY_STATUSES.includes(meta.status)) {
    errors2.push(error("status", `must be one of: ${STORY_STATUSES.join(", ")}`));
  }
  if (!isString(meta.priority) || !PRIORITIES.includes(meta.priority)) {
    errors2.push(error("priority", `must be one of: ${PRIORITIES.join(", ")}`));
  }
  if (!isString(meta.complexity) || !COMPLEXITIES.includes(meta.complexity)) {
    errors2.push(error("complexity", `must be one of: ${COMPLEXITIES.join(", ")}`));
  }
  if (!isString(meta.created))
    errors2.push(error("created", "must be a string"));
  if (!isString(meta.updated))
    errors2.push(error("updated", "must be a string"));
  if (!isString(meta.author))
    errors2.push(error("author", "must be a string"));
  if (!isArray(meta.tags))
    errors2.push(error("tags", "must be an array"));
  if (isArray(meta.acceptance_criteria)) {
    for (let i = 0;i < meta.acceptance_criteria.length; i++) {
      const ac = meta.acceptance_criteria[i];
      if (!isString(ac.id))
        errors2.push(error(`acceptance_criteria[${i}].id`, "must be a string"));
      if (!isString(ac.status) || !AC_STATUSES.includes(ac.status)) {
        errors2.push(error(`acceptance_criteria[${i}].status`, `must be one of: ${AC_STATUSES.join(", ")}`));
      }
    }
  }
  if (isArray(meta.tasks)) {
    for (let i = 0;i < meta.tasks.length; i++) {
      ...
      if (!isString(task.id))
        errors2.push(error(`tasks[${i}].id`, "must be a string"));
      if (!isString(task.status) || !TASK_STATUSES.includes(task.status)) {
        errors2.push(error(`tasks[${i}].status`, `must be one of: ${TASK_STATUSES.join(", ")}`));
      }
    }
  }
  return { valid: errors2.length === 0, errors: errors2 };
}
// packages/core/src/knowledge/dimensions.ts
...
function suggestDimension(text) {
  const lower = text.toLowerCase();
  let bestDimension = "praxeology";
  let bestScore = 0;
...
// packages/core/src/knowledge/extractor.ts
function extractRawLearnings(body) {
  const learnings = [];
...
function assessScope(text, section) {
  const lower = text.toLowerCase();
  if (lower.includes("enterprise") || lower.includes("organization") || lower.includes("company-wide")) {
    return "enterprise";
  }
  if (lower.includes("department") || lower.includes("cross-team")) {
    return "department";
  }
  if (lower.includes("team") || lower.includes("convention") || lower.includes("standard") || lower.includes("always") || lower.includes("never")) {
    return "team";
  }
  return "repo";
}
function assessConfidence(text, section) {
  const lower = text.toLowerCase();
...
function transformLearnings(rawLearnings, ctx) {
  const items = [];
...
function extractKnowledge(retroBody, ctx) {
...
function validateTransition(story, targetStatus) {
...
function errorResult(error2, details) {
  return { ok: false, error: error2, ...details ? { details } : {} };
}
...
function commandValidate(storyPath) {
  try {
    const { meta } = readAndParse(storyPath);
    const result = validateStoryMeta(meta);
    if (!result.valid) {
      return errorResult("Invalid story", result.errors.map((e) => `${e.field}: ${e.message}`));
    }
    const storyMeta = meta;
    return successResult({
      id: storyMeta.id,
      status: storyMeta.status,
      ac_count: Array.isArray(storyMeta.acceptance_criteria) ? storyMeta.acceptance_criteria.length : 0,
...
function commandTransition(storyPath, targetStatus) {
  try {
    const { meta } = readAndParse(storyPath);
    const validationResult = validateStoryMeta(meta);
    if (!validationResult.valid) {
      return errorResult("Invalid story structure", validationResult.errors.map((e) => `${e.field}: ${e.message}`));
    }
    const storyMeta = meta;
    const from = storyMeta.status;
    const to = targetStatus;
    const canResult = canTransition(from, to);
    if (!canResult.allowed) {
      return errorResult(`Cannot transition from '${from}' to '${to}'`, [
        canResult.reason || "Invalid transition"
      ]);
    }
    const transitionResult = validateTransition(storyMeta, to);
...
function commandExtractKnowledge(retroPath, args) {
  try {
    ...
    if (existsSync(knowledgeDir)) {
      ...
      for (const file of files) {
        ...
        if (parsed && typeof parsed.meta.id === "string") {
          existingIds.push(parsed.meta.id);
        }
      }
    }
    const ctx = {
...
function commandNextId(configPath) {
  try {
...
function commandList(directory, args) {
  try {
...
var TEMPLATES_DIR = join2(import.meta.dir, "../templates");
function loadTemplate(relativePath) {
  return readFileSync2(join2(TEMPLATES_DIR, relativePath), "utf-8");
}
function configMd(ctx) {
...
class SwarmInstaller {
  ...
  ensureDirectories() {
    ...
    for (const dir of SWARM_DIRECTORIES) {
      const fullPath = join3(this.projectDir, dir);
    ...
    for (const dir of GITKEEP_DIRECTORIES) {
      const gitkeepPath = join3(this.projectDir, dir, ".gitkeep");
      if (existsSync2(gitkeepPath)) {} else {
        if (!this.dryRun) {
  ...
  ensureConfig() {
    ...
    const fullPath = join3(this.projectDir, relPath);
    if (existsSync2(fullPath)) {
      return [{ action: "skipped", path: relPath }];
    }
    if (!this.dryRun) {
      mkdirSync2(join3(this.projectDir, ".swarm"), { recursive: true });
  ...
  ensureTemplates() {
    ...
    for (const [relPath, content] of templates) {
      ...
      if (existsSync2(fullPath)) {
        ...
        if (!this.dryRun) {
          mkdirSync2(join3(this.projectDir, ".swarm/templates"), { recursive: true });
          Bun.write(fullPath, content);
        }
        results.push({ action: "created", path: relPath });
      }
  ...
  ensureAgentMemory() {
    ...
    for (const name of AGENT_NAMES) {
      ...
      if (existsSync2(fullPath)) {
        ...
        if (!this.dryRun) {
          mkdirSync2(join3(this.projectDir, `.claude/agent-memory/${name}`), { recursive: true });
  ...
  ensureGitignore() {
    ...
    let existingLines = [];
    if (existsSync2(fullPath)) {
      existingLines = readFileSync3(fullPath, "utf-8").split(`
`);
    }
...
`);
  ...
  resolveProjectName() {
    const pkgPath = join3(this.projectDir, "package.json");
    ...
    return basename2(this.projectDir);
  }
  groupPaths(paths) {
...
var TOOLS = [
  {
    name: "swarm_init",
    description: "Initialize SWARM SDLC in a project directory. Creates .swarm/ structure, quality gates, templates, and agent memory files.",
    inputSchema: {
      type: "object",
      properties: {
        project_dir: {
  ...
  {
    name: "swarm_validate",
    description: "Validate a SWARM story file. Checks frontmatter structure, required fields, and returns story metadata.",
    inputSchema: {
      type: "object",
      properties: {
        path: {
  ...
  },
  {
    ...
    description: "Check if a story status transition is allowed. Validates both structural transitions and semantic preconditions.",
    inputSchema: {
      type: "object",
      properties: {
        path: {
        ...
        target_status: {
          type: "string",
          description: "Target status to transition to (e.g., ideating, planned, executing, verifying, done, archived)"
        }
      },
  ...
  {
    name: "swarm_next_id",
    description: "Get the next story ID by reading and incrementing the counter in the SWARM config file.",
    inputSchema: {
      type: "object",
      properties: {
        config_path: {
  ...
  {
    name: "swarm_list",
    description: "List stories or knowledge items in a directory. Reads frontmatter from all .md files and optionally filters by status.",
    inputSchema: {
      type: "object",
      properties: {
        directory: {
          type: "string",
          description: "Absolute path to the directory to list (e.g., .swarm/backlog or .swarm/knowledge)"
        },
        status: {
          type: "string",
  ...
  },
  {
    ...
    description: "Extract knowledge items from a retrospective file. Parses the retro body, classifies items by E/Q/P dimension, and writes knowledge files.",
    inputSchema: {
      type: "object",
      properties: {
        retro_path: {
  ...
  {
    name: "swarm_status",
    description: "Check SWARM initialization status in a project directory. Returns which .swarm/ files and directories exist.",
    inputSchema: {
      type: "object",
      properties: {
        project_dir: {
...
async function handleToolCall(name, args) {
  switch (name) {
    ...
    case "swarm_validate": {
      const result = commandValidate(args.path);
      return {
    ...
    case "swarm_transition": {
      const result = commandTransition(args.path, args.target_status);
      return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        isError: !result.ok
      };
    ...
    case "swarm_next_id": {
      const result = commandNextId(args.config_path);
      return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        isError: !result.ok
      };
    ...
    case "swarm_list": {
      const statusArgs = [];
      if (args.status) {
        statusArgs.push("--status", args.status);
      }
      const result = commandList(args.directory, statusArgs);
      return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        isError: !result.ok
      };
    ...
    case "swarm_extract_knowledge": {
      const extraArgs = [];
      if (args.story_id) {
        extraArgs.push("--story-id", args.story_id);
      }
      if (args.repo) {
        extraArgs.push("--repo", args.repo);
      }
      const result = commandExtractKnowledge(args.retro_path, extraArgs);
      return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        isError: !result.ok
      };
    ...
    case "swarm_status": {
      ...
      for (const check of checks) {
        const present = existsSync3(join4(projectDir, check));
