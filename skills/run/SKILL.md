...
2. **Gate check:** Use the `swarm_validate` tool with `path` set to the absolute path of `.swarm/backlog/${STORY_ID}.md`.
...
2. **Gate check:** Use the `swarm_transition` tool with `path` and `target_status` set to `planned`.
   Verify the story has:
...
2. **Gate check:** Use the `swarm_transition` tool with `target_status` set to `done`.
   Verify:
